...
-n                 :   if given, then GitWCRev will error if the working\n\
...
\n\
GitWCRev reads the Subversion status of all files in a working copy\n\
...
bool InsertRevision(char* def, char* pBuf, size_t& index, size_t& filelength, size_t maxlength, GitWCRev_t* SubStat)
{
	...
	size_t hashlen = strlen(SubStat->HeadHashReadable);
	ptrdiff_t exp = 0;
	...
	memmove(pBuild, SubStat->HeadHashReadable, hashlen);
	filelength += Expansion;
	return true;
}
bool InsertRevisionW(wchar_t* def, wchar_t* pBuf, size_t& index, size_t& filelength, size_t maxlength, GitWCRev_t* SubStat)
{
	// Search for first occurrence of def in the buffer, starting at index.
	...
	std::wstring hash = CUnicodeUtils::StdGetUnicode(SubStat->HeadHashReadable);
...
bool InsertDate(char* def, char* pBuf, size_t& index,  size_t& filelength, size_t maxlength, time_t date_svn)
{
	// Search for first occurrence of def in the buffer, starting at index.
	if (!FindPlaceholder(def, pBuf, index, filelength))
	{
...
bool InsertDateW(wchar_t* def, wchar_t* pBuf, size_t& index, size_t& filelength, size_t maxlength, time_t date_svn)
{
	// Search for first occurrence of def in the buffer, starting at index.
	if (!FindPlaceholderW(def, pBuf, index, filelength))
	{
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	git_libgit2_init();
	// TODO: set all config files

	GetStatus(wc, SubStat);
	...
	size_t index = 0;
	while (InsertRevision(VERDEF, pBuf.get(), index, filelength, maxlength, &SubStat));
	index = 0;
	while (InsertRevisionW(TEXT(VERDEF), (wchar_t*)pBuf.get(), index, filelength, maxlength, &SubStat));

	index = 0;
	while (InsertRevision(VERDEFSHORT, pBuf.get(), index, filelength, maxlength, &SubStat));
	index = 0;
	while (InsertRevisionW(TEXT(VERDEFSHORT), (wchar_t*)pBuf.get(), index, filelength, maxlength, &SubStat));

	index = 0;
	while (InsertDate(DATEDEF, pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));
	index = 0;
	while (InsertDateW(TEXT(DATEDEF), (wchar_t*)pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));

	index = 0;
	while (InsertDate(DATEDEFUTC, pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));
	index = 0;
	while (InsertDateW(TEXT(DATEDEFUTC), (wchar_t*)pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));

	index = 0;
	while (InsertDate(DATEWFMTDEF, pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));
	index = 0;
	while (InsertDateW(TEXT(DATEWFMTDEF), (wchar_t*)pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));
	index = 0;
	while (InsertDate(DATEWFMTDEFUTC, pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));
	index = 0;
	while (InsertDateW(TEXT(DATEWFMTDEFUTC), (wchar_t*)pBuf.get(), index, filelength, maxlength, SubStat.HeadTime));
	...
	index = 0;
	while (InsertBoolean(ISINGIT, pBuf.get(), index, filelength, SubStat.bIsGitItem));
	index = 0;
	while (InsertBooleanW(TEXT(ISINGIT), (wchar_t*)pBuf.get(), index, filelength, SubStat.bIsGitItem));
